...
/*
 * Variant "split" — the same rose CTA band, but copy-left / button-right
 * (stacked on mobile) instead of centred.
 */
export function BelezaCTASplit({ data }: { data: unknown }) {
  const d = data as CtaData;
          ...
          >
            ...
            <div className="relative flex flex-col items-start gap-6 md:flex-row md:items-center md:justify-between">
              <div className="max-w-xl">
                <h2 className="text-2xl font-bold leading-tight md:text-4xl">{d.title}</h2>
                {d.subtitle && <p className="mt-3 max-w-xl text-base leading-relaxed text-white/85 md:text-lg">{d.subtitle}</p>}
              </div>
              <a
                href={d.buttonHref ?? "#lead_form"}
                className="inline-flex shrink-0 items-center gap-2 rounded-full bg-white px-7 py-3 text-sm font-bold text-[color:var(--beleza-branding)] transition-transform hover:-translate-y-0.5"
              >
                {d.buttonLabel}
                <ArrowIcon className="h-4 w-4" />
              </a>
