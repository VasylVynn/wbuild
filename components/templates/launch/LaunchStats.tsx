...
/*
 * Variant "cards" — the same grounded numbers as glass cards (instead of a bare
 * strip): each card centres a gradient value over a muted label.
 */
export function LaunchStatsCards({ data }: { data: unknown }) {
  const d = data as BlockProps["stats"];
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          {d.items.map((item, i) => (
            <div
              key={i}
              className="launch-glass flex flex-col items-center gap-2 rounded-2xl px-4 py-8 text-center"
              style={{ boxShadow: "var(--launch-shadow)" }}
            >
              <div
                className="launch-gradient-text text-4xl font-extrabold sm:text-5xl"
                style={{ filter: "drop-shadow(0 2px 20px color-mix(in oklch, var(--launch-brand-strong) 45%, transparent))" }}
              >
                {item.value}
              </div>
