...
5. **Distinctness-тест (acceptance для всієї механіки):** dev-скрипт генерує N=10 сайтів однієї вертикалі з однакових даних, збирає скріншот-сітку; критерій — жодна пара не читається як «той самий сайт» (палітра-сім'я/hero/шрифти візуально різні). Проганяється до шипу Фази 1 і після кожного нового бандла.
