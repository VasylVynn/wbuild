...
## СПРАВЖНІ ШАБЛОНИ — СТАТУС: ✅ перший шаблон shipped 2026-07-11 (журнал #48, рішення власника)

> **Поворот:** власник відхилив і скіни, і пакети як «переклад у наші токени» — вимога: згенерований сайт виглядає ТОЧНО як проект-джерело (той самий дизайн, шрифти, framer-motion анімації), а AI лише обирає шаблон → секції шаблону → пише контент. Архітектура: **порт секцій дослівно** (`components/templates/<id>/`) з контентом через наші zod-схеми блоків; `lib/templates/registry.ts`; блок несе `section` (id секції шаблону); ліміти в шаблоні — ПО СЕКЦІЇ, не по типу (features/howitworks/pricing живляться однією схемою services); шаблон володіє палітрою/шрифтами (може бути темним), wrapper скопить стилі (`.tpl-studio`); лід-форма і контакти достилізовані під шаблон (воронка недоторканна); `brand.templateId` переживає regenerate; превью `/admin/templates`. Пакети/скіни — фолбек для вертикалей без шаблонів (старі сайти працюють).
> **Перший шаблон: `studio`** (design-template-main, темний фіолетовий преміум; generic/lawyer/autoservice) — 10 секцій, framer-motion 12, Inter з кирилицею, noise. Верифіковано: рендер усіх секцій + CSS у прод-бандлі + форсована генерація (модель склала features+howitworks+pricing з одного типу). Наступні: порт 5 SMB-шаблонів (bakery/organic/wholefoods/construction/dental) тим самим конвеєром після затвердження студії власником.

## Дизайн-пакети — СТАТУС: ✅ shipped 2026-07-10 (журнал #47) → тепер ФОЛБЕК (див. журнал #48 вище)

> Рандомну пер-блок лотерею скінів у ГЕНЕРАЦІЇ замінено **дизайн-пакетами** (`lib/design/packs.ts`): пакет = тема-пресет (палітра+шрифти+радіуси) + фіксований скін на кожен тип блоку, вірний одному шаблону-джерелу. 6 пакетів: Крафтова пекарня, Органік-фреш, Світлий маркет, Потужний будівельний, Спокійна клініка, Студія-преміум (+6 нових тема-пресетів). **Модель обирає пакет** під настрій бізнесу зі списку сумісних із вертикаллю (фолбек — випадковий сумісний); пакет має пріоритет над темою; `brand.packId` зберігається і переживає «Перегенерувати». Редактор: пікер «Оформлення» перемикає пакет цілком (тема + всі скіни одним кліком), палітра окремо — другорядна опція; ручний вибір скіна блоку лишився. Пакети ніколи не призначають price-hiding скіни. Перевірено live: пекарня → bakery-craft, СТО → construction-bold, усі скіни на сторінці = мапі пакета.
