...
export const COMPOSITION_ARCHETYPE_DOCS: Record<CompositionArchetype, string> = {
  ...
  "story-first":
    "одразу за hero йде історія/«про нас» (story лише коли є 2–3 реальні фото для рядків — інакше веди історію текстовою секцією чи process); послуги — у середині сторінки.",
  "proof-first":
  ...
  editorial:
    "повільний редакційний ритм: hero → story → галерея → послуги (лише коли є реальні фото для story й галереї — інакше прозові секції без них); більше прози, менше карток.",
};
...
- ПАЛІТРА МАЄ ВИГЛЯДАТИ ДОРОГО (рішення власника 2026-08-12). Що це означає практично: bg і surface — НЕ чисті #ffffff/#000000, а глибокі чи теплі напівтони з відчутною, але стриманою різницею між собою; ОДИН насичений акцент замість кількох крикливих; ink — мʼякший за чистий чорний. Дешевить: базовий синій «як у всіх», сірий #f5f5f5 + чорний текст, два конкурентні акценти, неонові кольори поза нішами, де вони доречні.
- ТЕМНА ПАЛІТРА — повноцінний напрям, не виняток: для майстерень, барберень, автосервісів, юристів, вечірніх закладів глибоке темне тло з теплим акцентом часто виглядає дорожче за світле. Не обирай світле «за замовчуванням» — обирай під бізнес.
