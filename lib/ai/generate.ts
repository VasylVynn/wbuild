...
- Це МАТЕРІАЛИ, ПОВЕРХНІ, СВІТЛО й РОСЛИННІСТЬ саме цієї справи — не абстракція «взагалі». Загальна текстура без звʼязку з бізнесом дає гарну, але порожню картинку: грумінг → "soft clean towels and foam texture in warm light"; пекарня на заквасці → "flour dusted linen and rustic crust texture"; теніс → "clay court surface in low evening sun".
- Бери матеріал і поверхню із зібраних фактів (з чим працюють, у якій манері) — не вигадуй того, чого в них немає.
- ЗАБОРОНЕНО: люди й обличчя, тварини, впізнавані товари як «наші», будь-який текст/ціни/логотипи, приміщення, фасади й вітрини, які можна прийняти за їхнє реальне місце.
