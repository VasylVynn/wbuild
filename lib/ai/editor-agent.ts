...
export const toolInputSchemas = {
  ...
  // The DESIGN. Until now the agent had no tool that could touch the
  // stylesheet, so it honestly refused every «зміни колір кнопки» with «це
  // частина загального дизайн-шаблону» — true, and useless to the owner.
  // A natural-language instruction, not raw CSS: the sheet is rewritten by the
  // same stylist that writes it during generation, then goes through the SAME
  // compile door (lint → contrast → 60k clamp) before it can reach the draft.
...
const TOOL_DESCRIPTIONS: Record<ToolName, string> = {
  ...
  update_style:
    "Змінити ВИГЛЯД сайту: кольори, кнопки, фони, відступи, заокруглення, тіні, розміри тексту. Це справжня зміна стилю всього сайту — саме нею відповідай на «зроби кнопки іншими», «інший колір», «занадто просто виглядає». Одна інструкція = один перезапис стилю, тож описуй усе потрібне разом.",
  inspect_site:
...
export const TOOL_LABELS: Record<ToolName, string> = {
  ...
  update_style: "Перемальовую стиль сайту…",
...
- ВИГЛЯД САЙТУ ТИ ЗМІНЮЄШ. «Зроби кнопки іншими», «інший колір», «занадто просто виглядає», «більше повітря» — це update_style, а не відмова. Збери все прохання в ОДНУ інструкцію: кожен виклик переписує стиль цілком, тож два поспіль перекреслять один одного.
- Чого стиль не може: змінити КАРКАС — які секції існують, їхній порядок і розкладку (це блоки, і для них є свої інструменти), а також геометрію навігації, логотипа й форми заявки, яку каркас тримає навмисно. Якщо частину правки каркас не прийняв, інструмент скаже — не повторюй те саме вдруге.
