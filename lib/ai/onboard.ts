  ...
  input_schema: z.toJSONSchema(
    z.object({
      ...
      quickReplies: z
        ...
        .optional()
        .describe(
          "2–4 короткі чипи-відповіді на ТВОЄ поточне питання (1–4 слова), ЛИШЕ коли варіанти очевидні (типи бізнесу, «Так»/«Ні», «Пропустити»). Не давай для вільних відповідей (назва, телефон).",
        ),
    }),
  ),
...
const scrapeInstagramTool = {
  ...
  description:
    "Заглянути в Instagram-профіль бізнесу: витягнути опис, категорію, контакти-кандидати та фото. Викликай, щойно в розмові зʼявився хендл чи посилання на Instagram, або на прохання власника (можна й повторно). Спершу зроби скрейп, а вже тоді підсумовуй знайдене.",
  input_schema: z.toJSONSchema(
    z.object({
      handle: z
        .string()
        .describe("Хендл або посилання на Instagram-профіль (можна з @ чи https:// — я нормалізую)."),
      focus: z
        .string()
        .optional()
...
export function buildOnboardSystem(args: {
  ...
  const igLine = apifyEnabled
    ? "- Заглянути в Instagram бізнесу за посиланням чи хендлом — сам витягну опис, категорію, контакти-кандидати й фото (інструмент scrape_instagram). Можу зробити це повторно на прохання («пошукай ще раз телефон»)."
    : "";
  const igToolLine = apifyEnabled
    ? "- Посилання чи хендл Instagram у повідомленні (або прохання «візьми з інстаграма») → ОДРАЗУ виклич scrape_instagram сам, без зайвих питань. Спершу зроби скрейп і подивись результат, тоді підсумовуй."
    : "";
...
- Поки збираєш (status "collecting"), КОЖНА відповідь закінчується ОДНИМ конкретним питанням зі знаком «?». Без мета-фраз («зберігаю дані», «продовжимо») — одразу саме питання.
- quickReplies давай лише коли варіанти очевидні (тип бізнесу, Так/Ні, «Пропустити»); для вільних відповідей (назва, телефон, адреса) — не давай.
